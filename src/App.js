...
class App extends Component {
  ...
  addForMonitor = (companyInfo) => {
      ...
      axios.get(`https://cryptic-sands-63938.herokuapp.com/v2/quotes/${companyInfo}`)
      .then((rep) =>{
          if(this.state.aviod_Red.indexOf(rep.data.quotes.quote.symbol) === -1) {
            ...
            console.log("Session id is already there:"+this.state.session_connected);
  ...
  updateSearchPattren = (pattren) =>{
      ...
      axios.get(`https://cryptic-sands-63938.herokuapp.com/v2/search/${pattren}`)
      .then((rep)=>{
  ...
  add = (text)=>{
      ...
      //  console.log("DEBUG 0 :"+self.state.companies[0].quotes.quote.symbol);
        ...
        axios.post('https://cryptic-sands-63938.herokuapp.com/v2/session/stream')
        .then ((rep)=> {
        self.state.session_connected = rep.data;
        let final_url = `https://cryptic-sands-63938.herokuapp.com/v2/stream?sessionId=${rep.data}`;
        this.state.aviod_Red.map(element => (
          final_url = final_url + `&symbols=${element}`
        ))
        console.log("The value of final url is :"+final_url);
        if(self.state.stompClient !== null){
          console.log("unsubscribing");
          self.state.stompClient.unsubscribe(`/topic/${self.state.session_connected}`);
        ...
        .then((repeat) => {
          ...
          var socket = new SockJS('https://cryptic-sands-63938.herokuapp.com/quote-websocket');
          self.state.stompClient = Stomp.over(socket);
          self.state.stompClient.connect({}, function (frame) {
            console.log('Connected: ' + frame);
            self.state.stompClient.subscribe(`/topic/${rep.data}`,function (greeting) {
              //console.log("HELLO : "+JSON.parse(greeting.body).quotes.quote.symbol);
              self.setState(prevState => ({
                companies : prevState.companies.map(item => { 
                  console.log("item.symbol is :"+self.state.companies.length);
                  if(self.state.companies.length > 1)
                  return JSON.parse(greeting.body).quotes.quote[self.state.aviod_Red.indexOf(item.symbol)];
                  else
  ...
  render() {
    return (
      ...
      { this.state.companies.length === 0 ? console.log("Not Applicable") : <Example list = {this.state} somethingExtra = {this.state.extra_Data}/>}
      { this.state.circle === false ? console.log("SAAB pattern is :"+ this.state.pattren) :  <CircularProgress disableShrink style = {{position : 'fixed' , top : '50%', left: '50%' }} color = 'primary'> </CircularProgress>}
